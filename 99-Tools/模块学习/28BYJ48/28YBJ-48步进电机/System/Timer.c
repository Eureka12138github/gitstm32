...
*/	
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=1;//这句和下一句是配置优先级的为什么这样配我现在也不清楚
	NVIC_InitStructure.NVIC_IRQChannelSubPriority=1;
	/**
		PreemptionPriority抢占优先级高的（值小的），可以优先中断，占位优先级高的（值小的）可以优先排队
		比如现在抢占和占位都是1，但是如果有一个中断的抢占和占位优先级配置为0，那个中断就会比现在这个
		中断优先级高，那个中断可以优先执行，优先排队。
	**/
...
/*
定时器名称是规定好的，需要在start文件里面的mds里面找,中断频率要在上面的PSC与ARR设定，现在为1ms
