...
{
	uint8_t KeyNumber=0;
	if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_12)==0){ KeyNumber=1;}
	if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_13)==0){ KeyNumber=2;}
	if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_14)==0){ KeyNumber=3;}
	if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_15)==0){ KeyNumber=4;}
	return KeyNumber;
}
...
void Key_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;//选择需要配置的端口
